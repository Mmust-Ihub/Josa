...
function NewsCard(props) {
  ...
  const url = `/${category}/${slug}`; // Use id in the URL
  ...
  return (
    <Link to={url} className="group shadow- flex-1 flex flex-col  bg-[#fafafa] border rounded-lg hover:scale-105 transition-all duration-200 ease-in-out">
      ...
      <div className="flex flex-col  px-2 align-top ">
        <h1 className="text-gray-800 group-hover:text-purple-800 transition-colors duration-200  ease-in-out flex-1 text-[17px] font-bold pt-2">
            {title}
        </h1>
